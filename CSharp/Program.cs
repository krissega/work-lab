...
{
    // ========= GraalVM isolate =========
    [DllImport("BTransactionService.dll", CallingConvention = CallingConvention.Cdecl)]
    ...
    static void Main()
    {
        // Absolute paths en Windows
      string baseDir = @"C:\work-lab\CSharp";
      string jsonPath = Path.Combine(baseDir, "transaccion_example.json");
      string keyPath  = Path.Combine(baseDir, "keys", "aes256.key");
      string encPath  = Path.Combine(baseDir, "out", "tx.enc");
        ...
        Console.WriteLine(" GraalVM isolate created (Windows)");
